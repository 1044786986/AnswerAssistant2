    ...
    protected void openFloatMenu(final int location) {
        ...
        for (int j = 0; j < mChildMenuList.size(); j++) {
            ...
            distance = inAll_angle == 180 ? mOpenChildMenuR
                    : TransformationManager.dp2px(circleImageView.r * 2,mContext);
            //x,y子菜单在当前位置到目标位置的距离
...
//                            | WindowManager.LayoutParams.TYPE_SYSTEM_OVERLAY;
    ...
    private void initFloatChildMenu() {
        ...
        for (int i = 0; i < mChildMenuItemList.size(); i++) {
            mChildPosition = i;
            circleImageView = new CircleImageView(mContext,mChildMenuItemList.get(i).getLogo());
            setChildMenuClick(circleImageView,i);
            mChildMenuList.add(circleImageView);
        }
        mChildR = circleImageView.r * 2;
        mOpenChildMenuR = TransformationManager.dp2px(circleImageView.r,mContext);
    ...
    public static final class Builder{
        ...
        private List<FloatMenuItem> mFloatMenuItems = new ArrayList<>();
        ...
        public Builder setFloatMenuItem(List<FloatMenuItem> list){
